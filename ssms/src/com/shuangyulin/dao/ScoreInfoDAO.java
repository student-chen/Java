...
public class ScoreInfoDAO {
    ...
    /*学生查询自己的成绩信息*/
    public List<ScoreInfo> StudentQueryScoreInfoInfo(String studentNumber) {
    	Session s = null; 
        try {
            s = HibernateUtil.getSession();
            String hql = "From ScoreInfo scoreInfo where 1=1";
            if(null != studentNumber && !studentNumber.equals("")) hql += " and scoreInfo.studentNumber.studentNumber='" + studentNumber + "'";
            Query q = s.createQuery(hql); 
            List scoreInfoList = q.list();
            return (ArrayList<ScoreInfo>) scoreInfoList;
        } finally {
